          ...
          {filtered.length === 0 ? (
            ...
            <div className="space-y-4">
              {filtered.map(ev => {
                ...
                return (
                  <div key={ev.id} className="p-5 rounded-xl border" style={{ borderColor: '#e0e0ef' }}>
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {ev.start_datetime && <p className="text-xs text-gray-400 mt-1">📅 {new Date(ev.start_datetime).toLocaleString('en-KE')}</p>}
                      ...
                      <button onClick={() => deleteEvent(ev.id)} className="btn-danger text-xs px-3 py-1.5 flex-shrink-0">Delete</button>
                    </div>
                  </div>
                )
              })}
